...
while (!file_exists($config->config_data['pid_directory'].'/'.$device->device_data['name'].'.norespawn')) {
    ...
    if ($config->config_data['transcode_enable'] == 1) {
        exec($config->config_data['vlc_exec'].' '.$device->full_url().' vlc://quit --sout \'#transcode{vcodec='.$config->config_data['transcode_video_codec'].',vb='.$config->config_data['transcode_bitrate'].',fps='.$config->config_data['transcode_fps'].',scale='.$config->config_data['scale'].',acodec='.$config->config_data['transcode_audio_codec'].'}:std{access=file,mux='.$config->config_data['recording_file_mux'].',dst="'.$config->config_data['recording_directory'].'/'.$device->device_data['name'].'.'.date('ymdHis').$config->config_data['recording_file_extension'].'"}\' 2>&1 | tee '.$config->config_data['log_directory'].'/'.$device->device_data['name'].'.'.date('ymdHis').'.log & jobs -p > '.$config->config_data['pid_directory'].'/'.$device->device_data['name'].'.pid');
    } else {
